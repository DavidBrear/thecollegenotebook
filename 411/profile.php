	...
	<div id="profileContent">
				...
				<div id="right">
							...
							<?php
									...
									if(isset($_GET['id']))
												{
													$numFriends = mysql_num_rows(mysql_query('SELECT friend2 FROM friends WHERE (friend1 = '.$_GET['id'].' OR friend2 = '.$_GET['id'].') AND (pending != 1);', $connection411));
													$friends = mysql_query('SELECT friend1, friend2 FROM friends WHERE (friend1 = '.$_GET['id'].' OR friend2 = '.$_GET['id'].') AND (pending != 1) ORDER BY RAND() LIMIT 6;', $connection411) or print ("error getting friends");
													...
													<div class="pageTitle2Profile">('.mysql_num_rows($friends).' of '.$numFriends.')</div><br><div id="viewAllLink"><a href="friends?id='.$_GET['id'].'" style="margin: 110px auto;">View All Friends</a></div>';
													print '<table style="margin: 0px auto">';
													$counter = 0;
													while ($rowFiends = mysql_fetch_array($friends, MYSQL_ASSOC))
													{
														if ($counter == 0)
														{
														 	print '<tr>';
														}
														if ($_GET['id'] == $rowFiends['friend1'])
														{
															print '<td class="friend" align="center">';
															$friend = mysql_query("SELECT name, thumb_url from login, users WHERE login.id = users.id AND login.id = ".$rowFiends['friend2'].";", $connection411) or die ("error friend2".mysql_error());
															$friendRow = mysql_fetch_array($friend, MYSQL_ASSOC);
															print '<a href="profile?id='.$rowFiends['friend2'].'">';
															print '<img src="'.$friendRow['thumb_url'].'" /><br />';
															print $friendRow['name'].'</a>';
															print '</td>';
														}
														else if($_GET['id'] == $rowFiends['friend2'])
														{
															print '<td class="friend" align="center">';
															$friend = mysql_query("SELECT name, thumb_url from login, users WHERE login.id = users.id AND login.id = ".$rowFiends['friend1'].";", $connection411) or die ("error friend1".mysql_error());
															$friendRow = mysql_fetch_array($friend, MYSQL_ASSOC);
															print '<a href="profile?id='.$rowFiends['friend1'].'">';
															print '<img src="'.$friendRow['thumb_url'].'" /><br />';
															print $friendRow['name'].'</a>';
															print '</td>';
														}
							...
							<div class="box">
								...
								<table id="aboutMe">
										<?php
											$dataAbout = mysql_query('SELECT aim, phonenum, aboutme, login, email FROM users, login where users.id = login.id AND login.id = '.$_GET['id'].';', $connection411) or die(mysql_error());
											
											while($rowAbout = mysql_fetch_array($dataAbout, MYSQL_ASSOC))
											{
												if(isset($row['login']) && $row['login'] != '*NONE*' && $row['login'] != '')
											{
												print '<tr><td colspan="100%"><b><p>My URL:&nbsp;</p></b><a id="userDiv" href="/'.$row['login'].'">http://'.$_SERVER['HTTP_HOST'].'/'.$row['login'].'</a></td></tr>';
												
											}
											else
											{
												print '<tr><td colspan="100%"><b><p>My URL:&nbsp;</p></b><a id="userDiv" href="/'.$row['email'].'">http://'.$_SERVER['HTTP_HOST'].'/'.$row['email'].'</a></td></tr>';
											}
							...
							$friends = mysql_query('SELECT pending FROM friends WHERE (('.$_GET['id'].' = friend1 AND friend2 = '.$_COOKIE['user'].') OR (friend2 = '.$_GET['id'].' AND friend1 = '.$_COOKIE['user'].')) AND pending = 0;', $connection411);
							$auth = mysql_fetch_array($authentication, MYSQL_ASSOC);
							$hide = false;
							if ((isset($_COOKIE['AK']) && $_COOKIE['AK'] == $auth['auth_key'] && isset($friends) && (mysql_num_rows($friends) == 1)) || $_COOKIE['user'] == $_GET['id'])
							{
							...
							else
							{
								...
								$dataComments = mysql_query('SELECT id, userid, senderid, comment, date FROM comments where userid = '.$_GET['id'].' ORDER BY id DESC LIMIT 10;', $connection411) or die("ERROR418: ".mysql_error());
								while ($rowComments = mysql_fetch_array($dataComments, MYSQL_ASSOC))
								{
									...
									if ((($_GET['id'] == $_COOKIE['user']) || ($rowComments['senderid'] == $_COOKIE['user'])) && !$hide)
									{
						...
						<div id="userPicture">
							...
							if(!(isset($_GET['id']) && ($_GET['id'] == $_COOKIE['user'])))
							{
						...
						<div class="box">
								...
								<tr><td class="bl"></td><td>
							<?php
								$likedisData = mysql_query('SELECT likes, dislikes FROM users WHERE id='.$_GET['id'].';', $connection411) or die('error contact a system admin with this error: finder(169)');
						...
						<?php
							/*$ReviewCounter = 0;
							$reviews = mysql_query('SELECT subject, review, reviews.id from reviews, users, schools WHERE users.id = '.$_GET['id'].' AND users.school = schools.name AND schools.id = reviews.schoolid ORDER BY reviews.id DESC  LIMIT 10;', $connection411) or die (mysql_error());
