<?php
	...
	$data = mysql_query('select id, privacy FROM login where login = "'.$requestURI[1].'" OR email = "'.$requestURI[1].'";', $connection) or die('<script type="text/javascript"> window.location = "index.html"</script>');
	...
	if (!isset($row['id']))
	{
		header('Location: profile.php?id='.$_COOKIE['userid']); //if not make the user go home
	}
	...
	if(isset($_POST['commented']) && ($_POST['commented']))
	{
		mysql_query('INSERT INTO comments(userid, senderid, comment) VALUES('.$id.', '.$_COOKIE['userid'].', "'.$_POST['comment'].'");', $connection) or die("<script type='text/javascript'>window.location='login.php'</script>");
		print '<div class="success">Comment Submitted!</div>';
		print '<script type="text/javascript">document.scrollTop = 300;</script>';
	}
	if (isset($_GET['delete']) && $_GET['delete'])
	{
		if(isset($_COOKIE['userid']) && ($id['id'] == $_COOKIE['userid']))
		{
			mysql_query('DELETE FROM comments WHERE id='.$_GET['msgID'].' AND userid = '.$_COOKIE['userid'].';', $connection) or die("error deleting");
		}
		else if(isset($_COOKIE['userid']))
		{
			mysql_query('DELETE FROM comments WHERE id='.$_GET['msgID'].' AND senderid = '.$_COOKIE['userid'].';', $connection) or die("error deleting");
		}
	}
	...
	if(isset($_COOKIE['_sess']) && isset($_COOKIE['userid']) && $_COOKIE['userid'] == $id)
		{
		...
		else if(isset($_COOKIE['_sess']) && isset($_COOKIE['userid']) && $_COOKIE['userid'] != $id)
		{
			print '<div id="navigation">';
		...
		$data = mysql_query('select name FROM users WHERE id = '.$id.';', $connection);
		$name = mysql_fetch_array($data, MYSQL_ASSOC);
...
<script type="text/javascript">
	...
	document.title = '<?php print $name['name'] ?> - The College Notebook';
</script>
	...
	?>
						...
						<div id="right">
							...
							<?php
									...
									if(isset($id))
												{
													$numFriends = mysql_num_rows(mysql_query('SELECT friend2 FROM friends WHERE (friend1 = '.$id.' OR friend2 = '.$id.') AND (pending != 1);', $connection));
													$friends = mysql_query('SELECT friend1, friend2 FROM friends WHERE (friend1 = '.$id.' OR friend2 = '.$id.') AND (pending != 1) ORDER BY RAND() LIMIT 6;', $connection) or print ("error getting friends");
													...
													<div class="pageTitle2Profile">Friends ('.mysql_num_rows($friends).' of '.$numFriends.')</div><br><a href="friends.php?id='.$id.'">View All Friends</a>';
													print '<table>';
													$counter = 0;
													while ($row = mysql_fetch_array($friends, MYSQL_ASSOC))
													{
														...
														if ($id == $row['friend1'])
														{
															...
															print '<a href="profile.php?id='.$row['friend2'].'">';
														...
														else if($id == $row['friend2'])
														{
															...
															$friendRow = mysql_fetch_array($friend, MYSQL_ASSOC);
															print '<a href="profile.php?id='.$row['friend1'].'">';
															print '<img src="'.$friendRow['thumb_url'].'" /><br />';
															print $friendRow['name'].'</a>';
															print '</td>';
														}
							...
							<div id="aboutMeBox">
								<div class="pageTitle2Profile">About Me</div>
										<?php
											...
											$dataAbout = mysql_query('SELECT sex, aboutme, hometown, from_state FROM users where id = '.$id.';', $connection);
							...
							$friends = mysql_query('SELECT pending FROM friends WHERE (('.$id.' = friend1 AND friend2 = '.$_COOKIE['userid'].') OR (friend2 = '.$id.' AND friend1 = '.$_COOKIE['userid'].')) AND pending = 0;', $connection);
							$auth = mysql_fetch_array($authentication, MYSQL_ASSOC);
							if ((isset($_COOKIE['AK']) && $_COOKIE['AK'] == $auth['auth_key'] && isset($friends) && (mysql_num_rows($friends) == 1)) || $_COOKIE['userid'] == $id)
							{
								print '<div id="profileComment">
										...
										<a href="/viewComments.php?uid='.$id.'&mi=0&gid='.md5(rand()).'">See All Comments</a>
								</div>';
								print '<hr>';
							}
							...
							{
								print '<div id="profileComment">';
								print '<a href="addFriend.php?id='.$id.'">Friend this person to Comment</a>';
								print '</div>';
								print '<hr>';
							}
							else
							...
							else
							{
								...
								$dataComments = mysql_query('SELECT id, userid, senderid, comment, date FROM comments where userid = '.$id.' ORDER BY id DESC LIMIT 10;', $connection) or die("ERROR");
								while ($rowComments = mysql_fetch_array($dataComments, MYSQL_ASSOC))
								{
									...
									print '<br> <p class="commentText">'.nl2br($rowComments['comment']).'</p><br><br>';
									if (($id == $_COOKIE['userid']) || ($rowComments['senderid'] == $_COOKIE['userid']))
									{
										print '<br><a href="javascript:getData(\'http://www.thecollegenotebook.com/getComments.php\', \'id='.$id.'&msgID='.$rowComments['id'].'&delete=1\', \'commentsBox\');">Delete</a>&nbsp;|&nbsp;';
									}
									else
									{
										print '<br>';
						...
						<div id="userPicture">
							<?php
							$data = mysql_query('SELECT * FROM login, users WHERE login.id = users.id AND login.id = '.$id.';', $connection) or die("<script type='text/javascript'>window.location='login.php'</script>");
							$row = mysql_fetch_array($data, MYSQL_ASSOC);
							...
							<fieldset>
								...
								<table>
											...
											if(isset($row['login']) && $row['login'] != '*NONE*' && $row['login'] != '')
												...
												$dataAbout = mysql_query('SELECT altEmail, aim, phonenum FROM users where id = '.$id.';', $connection);
												while($rowAbout = mysql_fetch_array($dataAbout, MYSQL_ASSOC))
												{
						...
						<div id="adspace" style="text-align: -moz-center; text-align: center;">
							...
							switch($adRow['type'])
							{
								...
								case 'div':
								{
									...
									print $adRow['text2'];
									print '<br>';
									print '<a href="/createAd.php">Create your own Ad</a>';
									print '</div>';
								}break;
								case 'flash':
								{
						...
						?>
							...
							<?php
								$likedisData = mysql_query('SELECT likes, dislikes FROM users WHERE id='.$id.';', $connection) or die('error contact a system admin with this error: finder(169)');
						...
						<div id="refers">
							...
							<table border="0" cellpadding="0" cellspacing="0">
								<?php
									$referData = mysql_query('SELECT num_referals, num_reviews, num_ref_rev FROM users WHERE id = '.$id.';', $connection) or die(mysql_error());
						...
						<?php
							$ReviewCounter = 0;
							$reviews = mysql_query('SELECT subject, review, reviews.id from reviews, users, schools WHERE users.id = '.$id.' AND users.school = schools.name AND schools.id = reviews.schoolid ORDER BY reviews.id DESC  LIMIT 10;', $connection) or die (mysql_error());
							...
							{
								print '<a href="review.php?rid='.$reviewRow['id'].'">';
